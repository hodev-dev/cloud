...
enum StoreGameStatus {
   LOADING,
   IDLE,
   FAILED,
}

const GenrePage = ({ store }) => {
   const [games, setGames] = useState<any>([]);
   const [gameStatus, setGameStatus] = useState(StoreGameStatus.LOADING);
   ...
   useEffect(() => {
      Axios.get('/api/stores/' + store + '/?page=' + currentPage)
         .then((response: AxiosResponse) => {
            ...
            setGameStatus(StoreGameStatus.IDLE);
         })
         .catch((error: AxiosError) => {
            console.log({ error });
            setGameStatus(StoreGameStatus.FAILED);
         });
   }, [currentPage]);
   ...
   return (
      <>
         ...
         <NamedDivider name={'فروشگاه:' + ' ' + store} size={'text-lg font-medium'} dir={'rtl'} />
...
export async function getServerSideProps({ params }) {
   ...
   return {
      props: { store }, // will be passed to the page component as props
   };
}
