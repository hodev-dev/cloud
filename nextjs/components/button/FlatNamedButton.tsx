...
const FlatNamedButton = (props: any) => {
   ...
   return (
      <Link scroll={false} href={props.to ? props.to : '#'}>
         <div
            key={props.key}
            onClick={props.onClick}
            className={`flex flex-row items-center w-full h-14 border border-warmGray-800 border-t-0 btn border-r-0 outline-none focus:ring-0 focus:outline-none btn btn-white   ${props.selected === true ? props.selectBg : 'bg-black'} hover:${
               props.selectBg
            } cursor-pointer `}
         >
            {renderShape()}
            <div className={`${props.noIcon ? 'hidden' : 'flex items-center justify-center w-10 h-10 mr-5 text-center bg-black border border-warmGray-800 rounded-md'}`}>
               <props.icon size={18} className={`${props.iconColor}`} />
            </div>
            <p title={props.name} className={`mr-5 text-md  text-gray-300 `}>
               {props.name}
            </p>
         </div>
      </Link>
